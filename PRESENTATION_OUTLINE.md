...
Основание разделов: [реестр гипотез](knowledge/hypotheses-v2.md), [план PoC](implementation/poc/PLAN.md), [таймлайн](knowledge/roadmap.md), [план проверки](knowledge/discovery-plan.md), [решения](knowledge/decisions.md), [контекст-пак MTS](CONTEXT_PACK_MTS.md).
...
**Слайд 2.2. Схема потока.** Источник: [план PoC](implementation/poc/PLAN.md).
...
**Слайд 2.5. Порядок реализации.** Четыре среза с контрольными точками: подготовка документов и привязок → однократное ревью и адресный отчёт → профиль проекта и приоритеты → установка и экспертная проверка. Детали — [план PoC](implementation/poc/PLAN.md#план-реализации).
...
| --- | --- | --- |
| Собраны исходные материалы кейса | Постановка, транскрибация Q&A, тестовые документы, шаблоны и чек-лист обязательных пунктов; оригиналы сохранены без изменений | [MTS/README.md](MTS/README.md#источники-и-данные) |
...
| Проработаны гипотезы-проблемы | Основная формулировка и два альтернативных ракурса — со стороны разработки и со стороны тестирования | [Контекст-пак, раздел 1](CONTEXT_PACK_MTS.md#1-проблема) |
...
| Собраны первые свидетельства | Разбор Q&A с кейсодателем, портреты пользователя и покупателя, кандидаты на замечания M-01–M-09, обзор аналогов и внешних автопроверок требований | [user-and-buyer.md](MTS/user-and-buyer.md), [document-analysis.md](MTS/document-analysis.md), [analogs.md](knowledge/analogs.md) |
| Выбрана гипотеза решения для первой реализации | Простой анализ через переносимый навык: 1.1 + 2.1 + 3.1–3.2 + 4.1–4.2; границы и состав вне PoC зафиксированы | [D-10](knowledge/decisions.md), [план PoC](implementation/poc/PLAN.md) |
...
| --- | --- | --- |
| Даня | Организация работы, помощь в реализации, экономический расчёт | Ведение артефактов и задач, база знаний и реестр гипотез, сбор исходных затрат для расчёта эффекта |
...
- Решить, какие численные значения выносим на слайды: несогласованные пороги и оценки без основания не выносим.
