...
| Full release | FR-043, FR-050, SC-013–SC-014 | locked install; unit/contract/integration/migration/security/E2E; exact FR/SC evidence; diff guards for `apps/web/`, `client-materials/`, feature 001/002; temporary source archive physically excludes `client-materials/` and still passes locked package build plus public unit/contract suites | T009–T010, T100, T110–T120 |
...
- fresh feature 001 `run-demo` output for PoC mapping; no client copy.

The entire public suite and package build MUST pass in a checkout where `client-materials/` is absent. Private corpus smoke, if used, is opt-in through an explicit external path and must not print or persist client content.
