...
```bash
git diff --exit-code review-platform-contract-v1.0.1 -- apps/web client implementation/poc specs/001-review-data-spec-poc specs/002-target-review-platform
...
Private client artifacts are never required. Any optional private smoke uses a path outside common fixtures and emits no content.
...
`make release-check` is an owned Make target. It must rebuild from locked dependencies, scan fixtures/wheels/images/captured safe logs, verify protected path diffs, start a clean isolated Compose project, apply and health-check both business and Procrastinate schemas, run smoke/restart/duplicate-delivery/lease/stage-kill tests, verify default network-level egress denial, and shut down without deleting data outside its isolated test volumes. It also creates a temporary `git archive` source export governed by `.gitattributes`, asserts that `client-materials/` is absent, and runs the locked package build plus public unit/contract suites there; a package or runtime import of client files therefore fails the release gate.
