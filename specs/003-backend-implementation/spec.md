...
- **FR-043**: portable skill, common fixtures и release artifacts MUST быть organization-neutral и MUST не содержать MTS/client materials, local absolute paths или secrets. Locked package build и public unit/contract suites MUST дополнительно проходить в generated source checkout/archive, где каталог `MTS/` физически отсутствует, чтобы запрет включал runtime/import dependency, а не только content scan.
...
- **SC-013**: все обязательные contract, unit, integration, migration, security и end-to-end tests завершаются успешно из locked dependency state; locked package build и public unit/contract subset отдельно проходят в source export без каталога `MTS/`.
- **SC-014**: git diff feature не содержит изменений под `apps/web/`, `MTS/`, `specs/001-*` или `specs/002-*`; shared root contract меняется только additive preflight v1.0.2.
...
- Вся новая документация и fixtures являются общими и синтетическими; материалы MTS используются только как evidence при формулировке edge cases и не копируются.
