...
Клиентские короткие названия из [списка MTS](../../../MTS/model-candidates.md) не являются model ID или готовыми профилями. Checkpoint, квантование, фактический context window, JSON/reasoning и serving API остаются неизвестными до проверки точного подключения. Ни один кандидат и внешний провайдер здесь не выбран.
